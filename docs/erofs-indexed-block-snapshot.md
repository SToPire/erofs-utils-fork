...
1. 将每一代新产生的 exception payload 保存到独立的 EROFS 分发镜像；
2. 为每一代快照生成一个累计的 EROFS index-only 镜像；
3. 由 index-only 镜像把一个逻辑 upper 文件的不同数据块映射到不同代的分发镜像；
...
- 运行时读取任意逻辑块时，可以通过累计索引直接定位到拥有该块的分发镜像，无需逐层重放 dm exception chain。
- 历史快照和不同分支能够共享同一批不可变分发镜像。
- 分发数据使用 EROFS 镜像承载，并能进一步纳入 OCI 分发体系。
...
- EROFS 多 external-device 索引扩展的具体磁盘格式；
- EROFS 分发镜像和 index-only 镜像的 OCI artifact/layer 封装方式；
...
一个不可变 EROFS 数据镜像，仅承载从 `Δi` 中提取出的 exception payload。

`Di` 不是完整 upper 块设备，也不包含足以独立解释累计状态的信息。即使它在形式上可以作为 EROFS 镜像被打开，单独读取其中的 payload 文件也不能得到有意义的 upper 文件系统视图。
...
Ii[b] = location(U0, b)                         if owner(i, b) 不存在
        location(Dk, payload(k, b))             if owner(i, b) = k
```

这里的 `location` 至少包含 external-device 标识和设备内物理块地址。

定义 `Deps(Ii)` 为 `Ii` 的块映射实际引用的数据设备集合。它是 `{U0, D1, ..., Di}` 的子集：如果某个旧数据镜像的所有 payload 都已被后续层覆盖，则它仍可能被历史 index 使用，但不再属于 `Ii` 的运行时读取依赖。

### 6.1 最新层覆盖旧层

同一逻辑块可以在多个世代被修改。`Ii` 始终直接指向最后一次修改该块的 `Dk`。旧 payload 继续存在于旧分发镜像中，以保证历史 index 仍然可用，但不会出现在新视图的读取路径中。
...
这是本架构依赖的关键 EROFS 能力，也是后续需要扩展的索引语义。
...
3. 所有本世代写入只进入该 lineage 的活动 snapshot，不允许修改 `V(i-1)` 依赖的任何已有数据镜像。
...
4. 将这些完整映射块写入新的 EROFS 分发镜像 `Di`，并记录其物理位置；
5. 以 `I(i-1)` 为基础继承所有已有块映射；
6. 对 `Ei` 覆盖的逻辑范围更新映射，使其指向 `Di` 中的新映射块；
7. 生成累计 index-only 镜像 `Ii`；
8. 校验 `Ii` 能够与其依赖集合共同还原 `Vi`；
9. 发布 `Di`、`Ii` 及其依赖描述。
...
`Vi` 可以作为后继增量的 origin。由于 `Ii` 是一个可独立寻址的累计状态，多个新 COW store 也可以从同一个 `Vi` 分叉，形成多个共享历史数据镜像的快照分支。
...
```text
Mi(b) = location(Di, new_payload(b))    if b ∈ Ei
        M(i-1)(b)                       otherwise
```

其中 `Mi` 是 `Ii` 中对 `Vi` 的累计块映射，`new_payload(b)` 是按 EROFS 映射粒度生成的完整块；必要时它已经包含从 `V(i-1)` 补齐的边界字节。
...
1. 根据快照描述取得 OCI lower、`Ii` 和 `Deps(Ii)`；
...
运行时的语义入口始终是 `Ii`，而不是任何一个 `Di`。如果缺失 `Ii` 或 `Deps(Ii)` 中的任一数据设备，该快照都不完整，backend 应拒绝建立视图，而不是返回部分数据。旧 lineage 中未被 `Ii` 引用的数据镜像不属于恢复 `Vi` 的必要读取依赖，但可能仍被历史快照或其他分支引用。
...
- 其业务数据只包含本代 exception payload，以及映射粒度不对齐时为形成完整 EROFS 数据块所必需的边界数据；镜像仍可包含必要的 EROFS 容器元数据；
...
- 描述完整 `Vi`，而不是只描述 `Δi`；
- 把逻辑块直接映射到最终拥有者；
- 保存 stable device ID、块地址及必要的文件元数据；
- 本身不承载 upper payload；
- 是恢复、分支和运行时读取的稳定入口。

这种分离使数据镜像可以保持增量和不可变，同时让每个快照仍然拥有一个元数据自足的累计逻辑描述；实际读取仍依赖 `Deps(Ii)`。

`Ii` 内的 device ID 和块地址是数据读取的权威映射。快照 manifest 负责把 stable device ID 解析为内容寻址的实际 artifact，并同时绑定对应的 OCI lower；manifest 不能用另一套块映射覆盖 `Ii`。两者身份不一致时必须拒绝恢复。
...
4. `Ii` 必须绑定它引用的 external devices 的可靠身份，而不能仅依赖偶然的加载顺序。
5. 每个逻辑块只能解析到一个确定的最终位置，覆盖优先级由最新世代决定。
6. `Deps(Ii)` 中所有被 index 引用的 payload 必须存在，并满足块大小、对齐和边界要求。
7. upper 逻辑设备大小在一条快照链中保持稳定；扩容需要显式定义新的迁移语义。
8. dm chunk、EROFS block 和 Guest 逻辑扇区之间的换算必须无歧义。
9. `Ii` 发布时，其引用的所有数据镜像必须已经可用且不可变，且对应写入世代已有确定的 cutoff。
10. 一个 VM 快照必须关联正确的 OCI lower；不能只保存 upper index 而忽略 lower 身份。
11. 当前抽象模型把每个 exception 解释为确定的字节内容；discard 和全零写入先物化为普通 payload。引入 `ZERO`、`HOLE` 等特殊映射属于后续优化。

## 13. 失败处理与校验原则

- dm store header 无效、exception 重复、payload 越界或 origin 大小不匹配时，构建必须失败。
- index 引用的 device 缺失、身份不匹配或大小变化时，恢复必须失败。
- 分发镜像应先于引用它的 index 发布，避免产生可见但不可满足的累计视图。
- 发布流程应保证 `Ii` 与其依赖描述的一致性，不能让消费者观察到半更新状态。
- 选择旧的 `Ik` 即表示回滚到第 k 代；回滚不应改写任何已有分发镜像。
...
- 分发统一：数据和索引均可使用 EROFS 镜像作为承载格式。

### 14.2 代价与约束

- 需要扩展 EROFS，使一个文件的不同块能够引用不同 external device。
- backend 必须同时管理 index 和多份数据镜像，并验证依赖关系。
- 快照层数增加时，external-device 数量、index 元数据和依赖管理成本会增长。
- 单个分发镜像不具备独立恢复价值，运维工具必须以 index 为入口。
...
现阶段的 dm-persistent file-delta 支持，解决的是如何向 mkfs.erofs 表达：某个已存在的逻辑文件在本次增量构建中只修改了 dm exception 指定的范围。

它能够读取 persistent exception metadata，并将本层 payload 与上一代文件视图组合成新的逻辑内容。这一能力是构建 `Di` 和 `Ii` 所需的输入基础，但不是本方案的最终输出形态。

最终方案还需要：

- 从 exception ranges 生成业务数据仅包含本层 payload（以及必要边界数据）的 `Di`；
- 从 `I(i-1)` 继承未变化范围的映射；
- 为变化范围生成指向 `Di` 的新映射；
- 在一个 inode 内编码跨多个 external device 的块地址；
- 让 file backend 根据 `Ii` 合成完整 `Vi`。

因此，当前 patch 应被看作构建管线中的 dm 增量输入适配层，而不应被当作整个快照架构本身。

## 16. 后续需要单独设计的问题

以下问题应在后续文档中分别展开：

1. EROFS 混合 external-device 文件布局的磁盘格式与兼容性。
2. external-device 身份、编号、摘要和发现机制。
3. `U0`、`Di`、`Ii` 与 OCI manifest/artifact 的组织方式。
4. 在线创建 `Δi`、Guest quiesce、切换写层和故障恢复协议。
5. index 元数据规模、最大设备数量和读取缓存策略。
6. dm chunk 与 EROFS block 的最佳尺寸关系。
7. discard、全零块、部分 chunk 和设备尾部的表示方式。
8. 快照分支、引用计数、垃圾回收与链压缩。
9. 数据完整性、签名、加密及远端按需拉取。
10. 构建性能、启动延迟、随机读放大和存储节省的基准测试。

## 17. 总结

本方案将 microVM 根文件系统拆分为两个正交的层次：OCI/EROFS 提供文件级不可变 lower，块快照链提供 overlayfs upper 的可写历史。

每一代 dm snapshot 只作为构建输入。其 exception payload 被提取到独立、不可变的 EROFS 分发镜像 `Di`；累计 index-only 镜像 `Ii` 则通过 `Deps(Ii)` 组织出完整的逻辑 upper 块设备 `Vi`。分发镜像负责保存数据，index-only 镜像负责赋予数据位置和累计快照语义。

该设计的关键价值在于：数据按修改量增量分发，历史层可共享，而任意快照仍可通过累计 index 及其依赖闭包直接恢复，无需物化完整镜像，也无需在运行时逐层重放 dm exception chain。
